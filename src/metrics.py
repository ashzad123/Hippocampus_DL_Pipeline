...
def dice_coefficient(pred, target, num_classes=3, ignore_background=True):
    ...
    dice_scores["mean_dice"] = np.mean(
        [v for k, v in dice_scores.items() if k.startswith("class_")]
    )
...
def iou_score(pred, target, num_classes=3, ignore_background=True):
    ...
    # Mean IoU
    iou_scores["mean_iou"] = np.mean(
        [v for k, v in iou_scores.items() if k.startswith("class_")]
    )
