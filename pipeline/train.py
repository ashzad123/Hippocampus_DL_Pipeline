...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.criterion = CombinedLoss(
            weight_ce=config["weight_ce"], weight_dice=config["weight_dice"]
        )
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config["num_epochs"]):
            ...
            # Validate
            val_loss, val_ce, val_dice_loss, val_dice, val_metrics = self.validate(
                epoch
            )
        ...
        history_path = os.path.join(
            self.config["checkpoint_dir"], "training_history.json"
        )
