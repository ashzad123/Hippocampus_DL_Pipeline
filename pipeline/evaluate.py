...
def visualize_sample(image, label, prediction, sample_idx, output_dir):
    ...
    fig.suptitle(
        f'Sample {sample_idx + 1} - Mean Dice: {dice["mean_dice"]:.4f}',
        fontsize=16,
        fontweight="bold",
    )
...
def plot_training_history(checkpoint_path, output_dir):
    ...
    history_path = os.path.join(
        os.path.dirname(checkpoint_path), "training_history.json"
    )
...
def evaluate_pipeline(checkpoint_path, output_dir):
    ...
    visualize_predictions(model, val_loader, device, output_dir, num_samples=5)
